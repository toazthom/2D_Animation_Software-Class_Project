...
ViewEdit::ViewEdit(wxFrame* parent) :wxScrolledCanvas(parent, wxID_ANY)
{
    ...
    parent->Bind(wxEVT_COMMAND_MENU_SELECTED, &ViewEdit::OnEditMachines, this, XRCID("EditMachines"));
}
...
/**
 * Bring up the machine dialog boxes for the machines in the system
 * @param event The menu event
 */
void ViewEdit::OnEditMachines(wxCommandEvent &event)
{
    for (auto actor : *GetPicture())
    {
        std::shared_ptr<Drawable> drawable = actor->HitTest(wxPoint(actor->GetPosition()));
        if (drawable != nullptr)
        {
            drawable->ShowDialogBox(this);
        }
